...
<template>
  ...
  >
    <article class="mx-auto w-full max-w-2xl lg:mx-0 lg:max-w-lg">
      <time
        :datetime="featuredPost.datetime" class="block text-sm/6 text-gray-600"
      >{{ featuredPost.date }}
      </time>
      <h2
        id="featured-post" class="mt-4 text-pretty text-3xl font-semibold tracking-tight text-gray-900 sm:text-4xl"
      >
        {{ featuredPost.title }} </h2>
      <p class="mt-4 text-lg/8 text-gray-600">
        {{ featuredPost.description }} </p>
      <div
        class="mt-4 flex flex-col justify-between gap-6 sm:mt-8 sm:flex-row-reverse sm:gap-8 lg:mt-4 lg:flex-col"
      >
        <div class="flex">
          <a
            :href="featuredPost.href" aria-describedby="featured-post" class="text-sm/6 font-semibold text-secondary"
          >Continue reading <span aria-hidden="true">&rarr;</span></a>
        </div>
        <div class="flex lg:border-t lg:border-gray-900/10 lg:pt-8">
          <a
            :href="featuredPost.author.href" class="flex gap-x-2.5 text-sm/6 font-semibold text-gray-900"
          > <img
            :src="featuredPost.author.imageUrl" alt="" class="size-6 flex-none rounded-full bg-gray-50"
      ...
      <div class="-my-12 divide-y divide-gray-900/10">
        <article v-for="post in posts" :key="post.id" class="py-12">
          <div class="group relative max-w-xl">
            <time
              :datetime="post.datetime" class="block text-sm/6 text-gray-600"
            >{{ post.date }}
            </time>
            <h2
              class="mt-2 text-lg font-semibold text-gray-900 group-hover:text-gray-600"
            >
              <a :href="post.href"> <span class="absolute inset-0" /> {{ post.title }} </a>
            </h2>
            <p class="mt-4 text-sm/6 text-gray-600">{{ post.description }}</p>
          </div>
          <div class="mt-4 flex">
            <a
              :href="post.author.href" class="relative flex gap-x-2.5 text-sm/6 font-semibold text-gray-900"
            > <img
              :src="post.author.imageUrl" alt="" class="size-6 flex-none rounded-full bg-gray-50"
            /> {{ post.author.name }} </a>
          </div>
        </article>
      </div>
